...
export default function MyProfile() {
    ...
    useEffect(() => {
        axios.get('https://newsapp-dpv1.onrender.comdashcheck', {
            headers:{
        ...
        axios.post('http://localhost:1111/getprofile/', {username:username}).then((response)=>{
            console.log(response.data)
            let ans=response.data
            setFullname(ans.fullName)
            setBio(ans.bio)
            setAddress(ans.address)
            setDateofbirth(ans.dateofbirth)
            setEmail(ans.email)
            setPhone(ans.phoneNumber)
            setfLink(ans.fbLink)
            settLink(ans.twitterLink)
            setUsername(ans.userName)
            setiLink(ans.igLink)
            setgLink(ans.gitHubLink)
            setwLink(ans.website)
        })
    })
                      ...
                      <div className="card">
                        <div className="card-body">
                          <div className="d-flex flex-column align-items-center text-center">
                            <img src={imgg} alt="Admin" className="rounded" width="170"/>
                            <div className="mt-3">
                              <h4>{usernamee}</h4>
                              <p className="text-secondary mb-1">Full Stack Developer</p>
                              <p className="text-muted font-size-sm">{bio}</p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div className="card mt-3">
                        <ul className="list-group list-group-flush">
                          <li className="list-group-item d-flex justify-content-between align-items-center flex-wrap">
                            ...
                            <span className="text-secondary">{wLink}</span>
                          </li>
                          <li className="list-group-item d-flex justify-content-between align-items-center flex-wrap">
                            ...
                            <span className="text-secondary">{gLink}</span>
                          </li>
                          <li className="list-group-item d-flex justify-content-between align-items-center flex-wrap">
                            <h6 className="mb-0"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="feather feather-twitter mr-2 icon-inline text-info"><path d="M23 3a10.9 10.9 0 0 1-3.14 1.53 4.48 4.48 0 0 0-7.86 3v1A10.66 10.66 0 0 1 3 4s-4 9 5 13a11.64 11.64 0 0 1-7 2c9 5 20 0 20-11.5a4.5 4.5 0 0 0-.08-.83A7.72 7.72 0 0 0 23 3z"></path></svg>Twitter</h6>
                            <span className="text-secondary">{tLink}</span>
                          </li>
                          <li className="list-group-item d-flex justify-content-between align-items-center flex-wrap">
                            ...
                            <span className="text-secondary">{iLink}</span>
                          </li>
                          <li className="list-group-item d-flex justify-content-between align-items-center flex-wrap">
                            ...
                            <span className="text-secondary">{fLink}</span>
                          </li>
                        </ul>
                      </div>
                    </div>
                      ...
                      <hr/>
                          ...
                          <div className="row">
                            <div className="col-sm-12">
                              <button className='btn w-100' id='theBtn' onClick={()=>{navigate('/editprofile')}}>
                              Edit
                              </button>
                            </div>
                            <div className="col-sm-12">
                              <button className='btn w-100' id='theBtn' onClick={()=>{navigate('/')}}>
                              Back to home
                              </button>
                            </div>
                          </div>
